...
class SC0_COORD_TRUE_PARTNER(OnPolicyAlgorithm):
    """
    ...
    def set_reward_params(self, action_idx_to_encoding, marginal_probability):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            sum_normalize = sum(predicted_partner_action)
            conditional_probability = predicted_partner_action / sum_normalize
            ...
            for candidate_action in self.action_idx_to_encoding:
                ...
                obs_from_partner_perspective[len(obs_from_partner_perspective)-2] = ego_action_t[0]
                obs_from_partner_perspective[len(obs_from_partner_perspective)-1] = ego_action_t[1]

                new_partner_obs = np.expand_dims(np.array([obs_from_partner_perspective]), axis=2)
                tensor_partner_obs = Variable(th.Tensor(new_partner_obs)).to(device=self.device)

                # print("tensor_partner_obs", tensor_partner_obs.shape)
