...
class INFLUENCE_PPO_HARVEST_VECTOR_V0(OnPolicyAlgorithm):
    """
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # Predicting using game input
            # pdb.set_trace()
            # state_t1 = np.squeeze(new_obs, axis=1)
            state_t = np.squeeze(self._last_obs, axis=2)
            state_t = np.squeeze(state_t, axis=0)

            state_t1 = np.squeeze(new_obs, axis=2)
            state_t1 = np.squeeze(state_t1, axis=0)

            ego_action_t = copy.deepcopy(self.action_to_one_hot[actions[0]])
            # pdb.set_trace()
            # human_pred_input = np.concatenate([ego_action_t, state_t.flatten(), state_t1.flatten()], axis=0)  # length = (14,)

            # ego_action_t.extend([0,0,0])
            # ego_action_t = np.reshape(ego_action_t, (1, 10))
            # ego_action_t = np.broadcast_to(ego_action_t, (1, 10, 10))
            human_pred_input = np.concatenate([ego_action_t, state_t, state_t1], axis=0)  # length = (14,)
            human_pred_input = np.expand_dims(np.array([human_pred_input]), axis=0)
            # print('human_pred_input', human_pred_input.shape)
            # pdb.set_trace()
            ...
            #
            # input_x.extend(self.action_to_one_hot[partner_state])
            # input_x.extend(self.action_to_one_hot[ego_action])
            ...
            for candidate_action in self.action_to_one_hot:
                ...
                # candidate_human_pred_input = np.concatenate([ego_action_t, state_t[:-1], state_t1[:-1]], axis=0)  # length = (14,)
                # candidate_human_pred_input = np.expand_dims(np.array([candidate_human_pred_input]), axis=2)
                # pdb.set_trace()
                ...
                candidate_human_pred_input = np.concatenate([ego_action_t, state_t, state_t1], axis=0)  # length = (14,)
            ...
            # pdb.set_trace()
            if self.desired_strategy is None:
                if current_position in self.desired_strategy:
                    desired_action_prob = self.desired_strategy[current_position]

                    mi_guiding_divergence = sum(rel_entr(conditional_probability, desired_action_prob))

                else:
                    mi_guiding_divergence = 10
            ...
            rewards[0] = self.transform_influence_reward(env_reward, mi_divergence, mi_guiding_divergence)

            new_reward = rewards[0]
            old_reward = env_reward
            # assert new_reward == env_reward
            ...
            record_ego_action = clipped_actions[0]
            # record_partner_action = state_t1
            record_partner_action = infos[0]['partner_action']['player_2']

            # if record_partner_action in self.action_to_one_hot and record_ego_action in self.action_to_one_hot:

            # pdb.set_trace()
            if record_ego_action in self.action_to_one_hot and record_partner_action in self.action_to_one_hot:
                record_ego_action = copy.deepcopy(self.action_to_one_hot[record_ego_action])
                record_partner_action = copy.deepcopy(self.action_to_one_hot[record_partner_action])
